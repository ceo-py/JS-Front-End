...
function solve() {
    ...
    const resetFields = (inputs) => inputs.forEach(x => x.value = '')
    ...
    const changeBtnFunc = (e) => {
        ...
        weatherTracker.editDate = [...currentContainer.querySelectorAll('.container h2, .container h3')].map(x => x.textContent)
        Object.values(weatherTracker.inputFields).forEach((x, i) => x.value = weatherTracker.editDate[i])
    ...
    const loadBtnFunc = () => {
        ...
        fetch(weatherAPI).then(x => x.json())
            .then(o => Object.values(o).forEach(x => {
                weatherTracker.list.innerHTML += `
                    <div class="container">
                        <h2>${x.location}</h2>
                        <h3>${x.date}</h3>
            ...
            .then(() => {
                [...weatherTracker.list.querySelectorAll('button')].forEach((x, i) => x.addEventListener('click', i % 2 === 0 ? changeBtnFunc : deleteBtnFunc))
                weatherTracker.addWeatherBtn.disabled = false
                weatherTracker.editWeatherBtn.disabled = true
            })
    }
