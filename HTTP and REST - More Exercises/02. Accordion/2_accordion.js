function solution() {
    ...
    const changeToggleInfo = (e, styleType, btnText) => {
        const parentElement = e.parentElement.parentElement
        const hiddenFieldData = parentElement.querySelector('p')
        hiddenFieldData.parentElement.style.display = styleType
        e.textContent = btnText
    }


    const toggleInfo = (e) => {
        if (e.textContent === 'More') changeToggleInfo(e, 'inline-block', 'Less')
        else changeToggleInfo(e, 'none', 'More')

    }

    fetch(articlesAPI).then(x => x.json())
        .then(l => l.forEach(x => {
            fetch(`${detailsAPI}${x._id}`).then(o => o.json())
                .then(e => {
                                    mainSection.innerHTML += `
        <div class="accordion">
            <div class="head">
                <span>${x.title}</span>
                <button class="button" id="${x._id}" onclick="funcJs.toggleInfo(this)">More</button>
            </div>
            <div class="extra">
                <p>${e.content}</p>
            </div>
